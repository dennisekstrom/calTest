 ...
 * similar to the Microsoft Outlook / Windows Vista calendar. See: <img src=
 * 'http://www.microsoft.com/library/media/1033/athome/images/moredone/calendar.gif'/
 * >
 * <p>
...
public class DayViewLayoutStrategy {
	...
	public ArrayList<AppointmentAdapter> doLayout(List<Appointment> appointments, int dayIndex,
		...
		int intervalsPerHour = settings.getSettings().getIntervalsPerHour(); // 30
																				// minute
																				// intervals
		float intervalSize = settings.getSettings().getPixelsPerInterval(); // 25
																			// pixels
																			// per
																			// interval
			...
			for (TimeBlock block : timeBlocks) {
				...
				if (block.intersectsWith(apptCell)) {
					...
					if (groupEndIndex < startBlock.getOrder()) {
						...
						// System.out.println("new group at: " +
						// groupStartIndex);
						groupMaxColumn = 0;
					}
		...
		// last step is to calculate the adjustment reuired for 'multi-day' /
		// multi-column
		...
		for (AppointmentAdapter apptCell : appointmentCells) {
			...
			float left = (float) apptCell.getColumnStart()
					/ (float) apptCell.getIntersectingBlocks().get(0).getTotalColumns() * 100;
			...
			apptCell.setHeight((float) apptCell.getIntersectingBlocks().size()
			...
			float blockEnd = timeBlocks[apptCell.getCellStart() + apptCell.getCellSpan() - 1]
					.getEnd();
			...
			// appt.formatTimeline(apptCell.getCellPercentStart(),
			// apptCell.getCellPercentFill());
		}
	...
	public int doMultiDayLayout(ArrayList<Appointment> appointments,
			ArrayList<AppointmentAdapter> adapters, Date start, int days) {
		...
		for (AppointmentAdapter adapter : adapters) {
			...
			boolean isStart = true; // indicates if current column is
			...
			for (int i = 0; i < dateList.size(); i++) {
				...
				// " for start: " + adapter.getAppointment().getStart() +
			...
			// now we set the row, which cannot be more than total # of
			// appointments
			for (int x = 0; x < adapters.size(); x++) {
				...
				for (int y = adapter.getColumnStart(); y <= adapter.getColumnStart()
					...
					try {
						...
						if (rowMap.containsKey(x)) {
							...
							// x+"] is occupied for day [" + y+"]");
						} else {
							// isRowOccupied = false;
							break; // break out of loop, nothing found in row
									// slot
						}
					} catch (Exception ex) {
						System.out.println("Exception: y=" + y + " x=" + x + " adapters.size="
								+ adapters.size() + " start=" + adapter.getAppointment().getStart()
								+ " end=" + adapter.getAppointment().getEnd().toString());
					}
				}
				...
				if (!isRowOccupied) {
					...
					// float left = (float) adapter.getColumnStart() / (float)
					// apptCell.getIntersectingBlocks().get(0).getTotalColumns()
					// * 100;
					adapter.setWidth(width);
					adapter.setLeft(left);
					adapter.setTop(top);
					adapter.setHeight(20);
					// System.out.println("set appointment [" + appt.getTitle()
					// + "]  layout left: " + left + " top: " + top + " width: "
					// + width);
					break;
				}
			}

			// System.out.println("multi-day layout -- title: \"" +
			// adapter.getAppointment().getTitle() + "\" | col start: " +
			// adapter.getColumnStart() + " | colspan: " +
			// adapter.getColumnSpan() + " | row: " + adapter.getCellStart() +
			// " | start date: " + adapter.getAppointment().getStart() +
