...
/**
 * The DayGrid draws the grid that displays days / time intervals in the body of
 * the calendar.
 * 
 * @author Brad
 * 
...
public class DayViewGrid /* Impl */extends Composite {
	...
	// private int intervalsPerHour = settings.getIntervalsPerHour();//2; //30
	// minute intervals
	// private float intervalSize = settings.getPixelsPerInterval();//25f; //25
	// pixels per interval

	public DayViewGrid(HasSettings settings) { // was DayViewGridImpl
		initWidget(grid);
		this.settings = settings;
		// System.out.println("DayViewGrid loaded, with offset = " +
		// impl.getBorderOffset());
	...
	public void build(int workingHourStart, int workingHourStop, int days) {
		...
		int intervalsPerHour = settings.getSettings().getIntervalsPerHour();// 2;
																			// //30
																			// minute
																			// intervals
